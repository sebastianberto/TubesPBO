...
public class DriverScreen extends JFrame implements ActionListener{
    ...
    private JButton listorderbutton,driverprofilebutton,logoutbutton, donebutton, cancelbutton;
    ...
    public DriverScreen() {
        ...
        if(DriverManager.getInstance().getDrivers().getStatus().equals("Ada orderan")){
            ...
            for(int i = 0; i < listPesanan.size(); i++){
                if(listPesanan.get(i).getDriver().getId_driver() == DriverManager.getInstance().getDrivers().getId_driver()){
                    for(int j = 0; j < listPesananFood.size(); j++){
                        if(listPesanan.get(i).getId_pesanan() == listPesananFood.get(j).getId_pesanan() && listPesananFood.get(j).getStatusfood()== 2){
                            cekFood = true;
                            pesananFood = listPesananFood.get(j);
                        }
                    }
                    for(int j = 0; j < listPesananOjek.size(); j++){
                        if(listPesanan.get(i).getId_pesanan() == listPesananOjek.get(j).getId_pesanan() && listPesananOjek.get(j).getStatuspesanan() == 2){
                            cekOjek = true;
            ...
            donebutton.addActionListener(new ActionListener(){
                public void actionPerformed(ActionEvent e){
                    if(cekFood){
                        ...
                        ctrl.updateStatusPesananFood(1, pesananFood.getID_PesananFood());
                        DriverManager.getInstance().getDrivers().setStatus("Tidak ada orderan");
                    }else if(cekOjek){
                        ctrl.updateStatusPesananOjek(1, pesananOjek.getId_pesananojek());
                        DriverManager.getInstance().getDrivers().setStatus("Tidak ada orderan");
                    }
                    JOptionPane.showMessageDialog(null, "Pesanan Selesai!!", "Information", JOptionPane.INFORMATION_MESSAGE);
                    ctrl.updateStatusDriver("Tidak ada orderan", DriverManager.getInstance().getDrivers().getId_driver());
            ...
            cancelbutton.addActionListener(new ActionListener(){
                public void actionPerformed(ActionEvent e){
                    if(cekFood){
                        ctrl.updateStatusPesananFood(3, pesananFood.getID_PesananFood());
                        DriverManager.getInstance().getDrivers().setStatus("Tidak ada orderan");
                    }else if(cekOjek){
                        ctrl.updateStatusPesananOjek(3, pesananOjek.getId_pesananojek());
                        DriverManager.getInstance().getDrivers().setStatus("Tidak ada orderan");
                    }
